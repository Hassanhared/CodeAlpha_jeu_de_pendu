import random
mots = ["python", "java", "html", "impossible"]
...
while tentative_restant > 0 and not trouve:
    ...
    if lettre in lettres_trouves:
        print(f"vous avez déja proposé cette lettre {lettre}\n")
        tentative_restant -= 1
    ...
    if lettre in mot_secret:
        ...
        tentative_restant -= 1
        print(f"Mauvaise reponse, cette lettre {lettre} n'est pas dans le mot:"
              f" il vous reste {tentative_restant} tentative")
    affichage = ""
    for l in mot_secret:
